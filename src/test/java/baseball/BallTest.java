...
class BallTest {
	...
	void constructor(int value) {
		...
		// then
		assertThat(ball.getNumber()).isEqualTo(new Number(value));
	}
	...
	void constructor_fail(int value) {
		// given & when & then
		assertThatThrownBy(() -> new Ball(value)).isInstanceOf(IllegalArgumentException.class);
	}
	...
	void equals(int value) {
		// given & when & then
		assertThat(new Ball(value)).isEqualTo(new Ball(value));
	}
}
